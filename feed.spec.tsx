...
test.describe(`Feed`, () => {
  test(`This test cases verifies that the feed is created correctly`, async ({ page }) => {
    await it.step(`1. Navigate to \`/feed.xml\``, async () => {
      await runManualStep(page, `1. Navigate to \`/feed.xml\``);
    });

    await it.step(`2. Verify that the following metadata is set`, async () => {
      await runManualStep(page, `2. Verify that the following metadata is set`);
    });

    await it.step(`3. Verify roughly that the feed items seem legitimate`, async () => {
      await runManualStep(page, `3. Verify roughly that the feed items seem legitimate`);
    });
