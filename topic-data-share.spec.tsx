...
test.describe(`Topic Data Share`, () => {
  test(`Verifies that topic data states (considering filtering / search) is shareable / referenceable via link.`, async ({ page }) => {
    await it.step(`1. Go to: \`/topics/RPP/data\``, async () => {
      await runManualStep(page, `1. Go to: \`/topics/RPP/data\``);
    });

    await it.step(`2. Search for _Zurich OR Berlin_`, async () => {
      await runManualStep(page, `2. Search for _Zurich OR Berlin_`);
    });

    await it.step(`3. Click on Filter _Compiling Agency_`, async () => {
      await runManualStep(page, `3. Click on Filter _Compiling Agency_`);
    });

    await it.step(`4. Select _Private Sector_ as the filter value`, async () => {
      await runManualStep(page, `4. Select _Private Sector_ as the filter value`);
    });
    ...
    await it.step(`8. Click on _8_`, async () => {
      await runManualStep(page, `8. Click on _8_`);
    });

    await it.step(`9. Click on _Relevance_ (Sort Dropdown)`, async () => {
      await runManualStep(page, `9. Click on _Relevance_ (Sort Dropdown)`);
    });

    await it.step(`10. Select _Country_`, async () => {
      await runManualStep(page, `10. Select _Country_`);
    });

    await it.step(`11. Make sure _Descending Order_ is selected`, async () => {
      await runManualStep(page, `11. Make sure _Descending Order_ is selected`);
    });

    await it.step(`12. Click on Share`, async () => {
      await runManualStep(page, `12. Click on Share`);
    });

    await it.step(`13. Click on Copy / Input`, async () => {
      await runManualStep(page, `13. Click on Copy / Input`);
    });

    await it.step(`14. Create new Tab, Paste URL and hit ENTER`, async () => {
      await runManualStep(page, `14. Create new Tab, Paste URL and hit ENTER`);
    });

    await it.step(`15. Select the last item in the time series list by checking the checkbox on the right hand side of the card`, async () => {
      await runManualStep(page, `15. Select the last item in the time series list by checking the checkbox on the right hand side of the card`);
    });

    await it.step(`16. Repeat steps 12-14`, async () => {
      await runManualStep(page, `16. Repeat steps 12-14`);
    });
