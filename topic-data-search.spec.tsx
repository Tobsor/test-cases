...
test.describe(`Topic Data Search`, () => {
  test(`Verifies that the topic data search bar filters the data items correctly`, async ({ page }) => {
    await it.step(`1. Navigate to \`/topics/DSS/data\``, async () => {
      await runManualStep(page, `1. Navigate to \`/topics/DSS/data\``);
    });

    await it.step(`2. Identify one item in the list (e.g. \`Malta - Debt securities, asset price-linked issued by NFC, in all markets at all original maturities denominated in all currencies at market value stocks\`)`, async () => {
      await runManualStep(page, `2. Identify one item in the list (e.g. \`Malta - Debt securities, asset price-linked issued by NFC, in all markets at all original maturities denominated in all currencies at market value stocks\`)`);
    });

    await it.step(`3. Type the series key of the identified item in the search input (e.g. \`Q.N.MT.XW.S11.S1.N.L.LE.F3VRC.T._Z.EUR._T.M.V.N._T\`) and press ENTER`, async () => {
      await runManualStep(page, `3. Type the series key of the identified item in the search input (e.g. \`Q.N.MT.XW.S11.S1.N.L.LE.F3VRC.T._Z.EUR._T.M.V.N._T\`) and press ENTER`);
    });
    ...
    await it.step(`5. Type \`Switzerland\` in the search input`, async () => {
      await runManualStep(page, `5. Type \`Switzerland\` in the search input`);
    });

    await it.step(`6. Click onto the \`Switzerland\` suggestion`, async () => {
      await runManualStep(page, `6. Click onto the \`Switzerland\` suggestion`);
    });

    await it.step(`7. Hit F5 in the browser`, async () => {
      await runManualStep(page, `7. Hit F5 in the browser`);
    });
