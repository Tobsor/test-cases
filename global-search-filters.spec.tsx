...
test.describe(`Global Search Filters`, () => {
  test(`Verifies that the search bar features all filters and the filters function in a way to find specific data`, async ({ page }) => {
    ...
    await it.step(`2. Click on the chip _Payment statistics_`, async () => {
      await runManualStep(page, `2. Click on the chip _Payment statistics_`);
    });

    await it.step(`3. Click on _Data set_ filter`, async () => {
      await runManualStep(page, `3. Click on _Data set_ filter`);
    });

    await it.step(`4. Select _CPMI institutions (T3)_`, async () => {
      await runManualStep(page, `4. Select _CPMI institutions (T3)_`);
    });

    await it.step(`5. Close filter`, async () => {
      await runManualStep(page, `5. Close filter`);
    });

    await it.step(`6. Click on _Reporting country_ filter`, async () => {
      await runManualStep(page, `6. Click on _Reporting country_ filter`);
    });

    await it.step(`7. Click on _List view_`, async () => {
      await runManualStep(page, `7. Click on _List view_`);
    });

    await it.step(`9. Click on _Tree view_`, async () => {
      await runManualStep(page, `9. Click on _Tree view_`);
    });

    await it.step(`10. Click on _G8_ (in tree view) G8 is checked`, async () => {
      await runManualStep(page, `10. Click on _G8_ (in tree view) G8 is checked`);
    });

    await it.step(`11. Expand _G8_`, async () => {
      await runManualStep(page, `11. Expand _G8_`);
    });

    await it.step(`12. Close filter`, async () => {
      await runManualStep(page, `12. Close filter`);
    });

    await it.step(`13. Click on _Measure_ filter`, async () => {
      await runManualStep(page, `13. Click on _Measure_ filter`);
    });

    await it.step(`14. Select the _Value_ filter option`, async () => {
      await runManualStep(page, `14. Select the _Value_ filter option`);
    });

    await it.step(`15. Note the number in the chip of _Value_`, async () => {
      await runManualStep(page, `15. Note the number in the chip of _Value_`);
    });

    await it.step(`16. Close the filter`, async () => {
      await runManualStep(page, `16. Close the filter`);
    });

    await it.step(`17. Click on _Timespan_`, async () => {
      await runManualStep(page, `17. Click on _Timespan_`);
    });

    await it.step(`18. Click on Years`, async () => {
      await runManualStep(page, `18. Click on Years`);
    });

    await it.step(`19. Select: 2012, 2013`, async () => {
      await runManualStep(page, `19. Select: 2012, 2013`);
    });

    await it.step(`20. Click on Reset`, async () => {
      await runManualStep(page, `20. Click on Reset`);
    });

    await it.step(`21. Close dropdown`, async () => {
      await runManualStep(page, `21. Close dropdown`);
    });

    await it.step(`22. Click on reset at the top right of filters section`, async () => {
      await runManualStep(page, `22. Click on reset at the top right of filters section`);
    });
