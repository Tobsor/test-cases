...
test.describe(`Global search Page structure`, () => {
  test(`Global search Page structure`, async ({ page }) => {
    await it.step(`1. Navigate to "/search"`, async () => {
      ...
      await expect(page.locator("main").getByRole("combobox", { name: "Search for time series" })).toHaveValue("exchange rate");
    ...
    await it.step(`2. Click on Timespan filter, select YTD and close the filter`, async () => {
      ...
      await expect(page.locator("main")).toContainText(/Timespan: \d{4}-01-01 - \d{4}-\d{2}-\d{2}/);
    });

    await it.step(`3. Click on \`Relevance\` button in the toolbar`, async () => {
      await page.locator("main").getByRole("button", { name: /Sort dropdown collapsed/ }).click();
      await expect(page.getByRole("menuitem").first()).toBeVisible();
    });

    await it.step(`4. Click on \`Topics\` suggestion`, async () => {
      await page.getByRole("menuitem", { name: /Topic|Topics/ }).first().click();
      await expect(page.locator("main").getByRole("button", { name: /Sort dropdown/ })).toBeVisible();
      await expect(page).toHaveURL(/sort=/);
    });

    await it.step(`5. Click on the sort icon button in the toolbar`, async () => {
      await page.locator("main").getByRole("button", { name: "Toggle sorting" }).click();
      await expect(page).toHaveURL(/sort=/);
    });

    await it.step(`6. Check the two first time series`, async () => {
      const checkboxes = page.locator("main").getByRole("checkbox", { name: "Select time series" });
      await checkboxes.nth(0).check();
      await checkboxes.nth(1).check();
      await expect(checkboxes.nth(0)).toBeChecked();
      await expect(checkboxes.nth(1)).toBeChecked();
      await expect(page.locator("main")).toContainText("2 selected");
      await expect(page.locator("main").getByText("Compare time series")).toBeVisible();
    });

    await it.step(`7. Check the checkbox in the toolbar`, async () => {
      await page.locator("main").getByRole("checkbox", { name: "Select all" }).check();
      await expect(page.locator("main").getByRole("checkbox", { name: "Select all" })).toBeChecked();
      await expect(page.locator("main")).toContainText(/20 selected/);
    });

    await it.step(`8. Hover on \`Compare time series\` button in the toolbar`, async () => {
      await page.locator("main button").filter({ hasText: "Compare time series" }).hover();
