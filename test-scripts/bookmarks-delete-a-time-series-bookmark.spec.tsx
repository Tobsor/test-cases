...
async function selectBookmarkedRow(page: Page): Promise<void> {
  ...
  const checkbox = page.getByRole("checkbox", { name: "Select time series" }).last();
  await expect(checkbox, "Bookmarked time series checkbox should be available").toBeVisible();
  await checkbox.check();
  await expect(checkbox, "Bookmarked time series should be selected").toBeChecked();
...
test.describe(`Bookmarks Delete a time series bookmark`, () => {
  test(`Bookmarks Delete a time series bookmark`, async ({ page }) => {
    ...
    await it.step(`5. Check the bookmarked time series`, async () => {
      await selectBookmarkedRow(page);
    });

    await it.step(`6. Click on \`Remove\` button in the toolbar`, async () => {
      await page.getByRole("button", { name: "Remove bookmarks" }).click();
      await expect(
        page.getByRole("tabpanel", { name: "Time series" }).getByRole("heading", { name: "No bookmarks saved yet" }),
        "Bookmark should be removed",
      ).toBeVisible();
      await expect(page.getByRole("tabpanel", { name: "Time series" }).getByRole("article")).toHaveCount(0);
    });
  });
});
