...
test.describe(`Topic Overview Page`, () => {
  test(`Verifies that the topic page of a given topic features the expected elements correctly.`, async ({ page }) => {
    await it.step(`1. Navigate to \`/topics/LBS\``, async () => {
      await runManualStep(page, `1. Navigate to \`/topics/LBS\``);
    });

    await it.step(`Expected 1. The topic features a heading section with a background image and a title referring to LBS`, async () => {
      await verifyExpected(page, `Expected 1. The topic features a heading section with a background image and a title referring to LBS`);
    });

    await it.step(`Expected 2. The heading section features the local tab navigation with the following items:`, async () => {
      await verifyExpected(page, `Expected 2. The heading section features the local tab navigation with the following items:`);
    });

    await it.step(`Expected 3. The page features an About section`, async () => {
      await verifyExpected(page, `Expected 3. The page features an About section`);
    });

    await it.step(`Expected 4. The page features a Metadata section`, async () => {
      await verifyExpected(page, `Expected 4. The page features a Metadata section`);
    });

    await it.step(`Expected 5. Commentary section follows with links to the latest and an overview of all commentaries`, async () => {
      await verifyExpected(page, `Expected 5. Commentary section follows with links to the latest and an overview of all commentaries`);
    });

    await it.step(`Expected 6. Methodology section is featured`, async () => {
      await verifyExpected(page, `Expected 6. Methodology section is featured`);
    });

    await it.step(`Expected 7. Research and publications is a section featuring accordion elements`, async () => {
      await verifyExpected(page, `Expected 7. Research and publications is a section featuring accordion elements`);
    });

    await it.step(`Expected 8. Glossary follows as a section`, async () => {
      await verifyExpected(page, `Expected 8. Glossary follows as a section`);
    });

    await it.step(`Expected 9. The page features an FAQs section`, async () => {
      await verifyExpected(page, `Expected 9. The page features an FAQs section`);
    });

    await it.step(`Expected 10. Related topics section is featured as the last section`, async () => {
      await verifyExpected(page, `Expected 10. Related topics section is featured as the last section`);
    });

    await it.step(`Expected 11. The page features a right hand side anchor navigation`, async () => {
      await verifyExpected(page, `Expected 11. The page features a right hand side anchor navigation`);
    });

    await it.step(`Expected 12. The sections are visually separated with odd/even colors (white/grey)`, async () => {
      await verifyExpected(page, `Expected 12. The sections are visually separated with odd/even colors (white/grey)`);
    });
