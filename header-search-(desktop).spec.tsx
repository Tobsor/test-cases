...
test.describe(`Header Search (Desktop)`, () => {
  test(`Verifies that the search in the header is usable and supports the user in finding the content he is looking for.`, async ({ page }) => {
    await it.step(`1. Go to any page but the home page e.g.: \`/help\``, async () => {
      await runManualStep(page, `1. Go to any page but the home page e.g.: \`/help\``);
    });

    await it.step(`2. Click on _Search_ in the main navigation`, async () => {
      await runManualStep(page, `2. Click on _Search_ in the main navigation`);
    });

    await it.step(`3. You can type without selecting the text field. Type anything.`, async () => {
      await runManualStep(page, `3. You can type without selecting the text field. Type anything.`);
    });

    await it.step(`4. Click on X to clear the entered text`, async () => {
      await runManualStep(page, `4. Click on X to clear the entered text`);
    });

    await it.step(`5. Click on _Explore_`, async () => {
      await runManualStep(page, `5. Click on _Explore_`);
    });

    await it.step(`6. Click on _Explore_ again`, async () => {
      await runManualStep(page, `6. Click on _Explore_ again`);
    });

    await it.step(`7. Click into the text field`, async () => {
      await runManualStep(page, `7. Click into the text field`);
    });

    await it.step(`8. Type: _germany_`, async () => {
      await runManualStep(page, `8. Type: _germany_`);
    });

    await it.step(`9. Click on X`, async () => {
      await runManualStep(page, `9. Click on X`);
    });

    await it.step(`10. Click into the search field`, async () => {
      await runManualStep(page, `10. Click into the search field`);
    });

    await it.step(`11. Type _Switzerland_`, async () => {
      await runManualStep(page, `11. Type _Switzerland_`);
    });

    await it.step(`12. Click on the first item in the suggestions`, async () => {
      await runManualStep(page, `12. Click on the first item in the suggestions`);
    });

    await it.step(`Expected 1. Close button is shown in the header after clicking _Search_ in header`, async () => {
      await verifyExpected(page, `Expected 1. Close button is shown in the header after clicking _Search_ in header`);
    });

    await it.step(`Expected 2. Exploration opened up correctly after step 5`, async () => {
      await verifyExpected(page, `Expected 2. Exploration opened up correctly after step 5`);
    });

    await it.step(`Expected 3. Clicking on first suggestion redirected to global search: \`/search?q=Switzerland\``, async () => {
      await verifyExpected(page, `Expected 3. Clicking on first suggestion redirected to global search: \`/search?q=Switzerland\``);
    });
