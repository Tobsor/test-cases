...
test.describe(`Sitemap`, () => {
  test(`This test case is about verifying that the sitemap is built correctly.`, async ({ page }) => {
    await it.step(`1. Visit [/sitemap.xml](https://dataportal.uat.bisinfo.org/sitemap.xml)`, async () => {
      await runManualStep(page, `1. Visit [/sitemap.xml](https://dataportal.uat.bisinfo.org/sitemap.xml)`);
    });

    await it.step(`2. Verify the following links are present`, async () => {
      await runManualStep(page, `2. Verify the following links are present`);
    });

    await it.step(`3. The domain of the URLs should be complete and represent the environment you are on. e.g. for UAT \`https://dataportal.uat.bisinfo.org/...\``, async () => {
      await runManualStep(page, `3. The domain of the URLs should be complete and represent the environment you are on. e.g. for UAT \`https://dataportal.uat.bisinfo.org/...\``);
    });
