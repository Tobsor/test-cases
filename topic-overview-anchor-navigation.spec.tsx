...
test.describe(`Topic Overview Anchor navigation`, () => {
  test(`Verifies that the anchor navigation on the topic overview works as expected`, async ({ page }) => {
    await it.step(`1. Navigate to /topics/LBS`, async () => {
      await runManualStep(page, `1. Navigate to /topics/LBS`);
    });

    await it.step(`Expected 1. In the right hand side anchor navigation, clicking on the item "Methodology" changes the route to /topic/LBS#methodology`, async () => {
      await verifyExpected(page, `Expected 1. In the right hand side anchor navigation, clicking on the item "Methodology" changes the route to /topic/LBS#methodology`);
    });

    await it.step(`Expected 2. The user is scrolled to the start of Methodology section of the page. The Title "Methodology" is fully visible on top of the visible screen area.`, async () => {
      await verifyExpected(page, `Expected 2. The user is scrolled to the start of Methodology section of the page. The Title "Methodology" is fully visible on top of the visible screen area.`);
    });

    await it.step(`Expected 3. Clicking F5 refreshes the page and the section is still visible.`, async () => {
      await verifyExpected(page, `Expected 3. Clicking F5 refreshes the page and the section is still visible.`);
    });
