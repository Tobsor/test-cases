...
test.describe(`AI Search Complex prompt example`, () => {
  test(`This test case should verify that a more complex prompt for the AI-Search yields expected results. Be aware that the behavior is non deterministic, the tester is tasked to decide whether the condition is met on his / her perception of whether the expected result is reasonably achieved.`, async ({ page }) => {
    await it.step(`1. Go to \`/search\``, async () => {
      await runManualStep(page, `1. Go to \`/search\``);
    });
    ...
    await it.step(`2. Enter the following prompt and press Enter: "How are the house prices and inflation rate in the countries Germany, Switzerland, Greece, Italy and Norwey? Provide annual data and the currencies CHF / Euro where you see fit"`, async () => {
      await runManualStep(page, `2. Enter the following prompt and press Enter: "How are the house prices and inflation rate in the countries Germany, Switzerland, Greece, Italy and Norwey? Provide annual data and the currencies CHF / Euro where you see fit"`);
    });

    await it.step(`3. Click on "Show reasoning"`, async () => {
      await runManualStep(page, `3. Click on "Show reasoning"`);
    });

    await it.step(`4. Click on the tab "Region reasoning"`, async () => {
      await runManualStep(page, `4. Click on the tab "Region reasoning"`);
    });

    await it.step(`5. Click on the tab "Dimenion reasoning"`, async () => {
      await runManualStep(page, `5. Click on the tab "Dimenion reasoning"`);
    });

    await it.step(`6. Verify which of the suggestion item features a dimension filter`, async () => {
      await runManualStep(page, `6. Verify which of the suggestion item features a dimension filter`);
    });
    ...
    await it.step(`8. Click on the item that features a dimension filter`, async () => {
      await runManualStep(page, `8. Click on the item that features a dimension filter`);
    });
