...
test.describe(`Topic Overview Page Filtering`, () => {
  test(`Ensures filtering the complete list of topics works.`, async ({ page }) => {
    await it.step(`1. Go to \`/topics\``, async () => {
      await runManualStep(page, `1. Go to \`/topics\``);
    });

    await it.step(`2. Click on the chip "Exchange rates"`, async () => {
      await runManualStep(page, `2. Click on the chip "Exchange rates"`);
    });

    await it.step(`Expected 1. Exchange rates chip is marked as selected`, async () => {
      await verifyExpected(page, `Expected 1. Exchange rates chip is marked as selected`);
    });

    await it.step(`Expected 2. Bilateral exchange rates is featured as a card`, async () => {
      await verifyExpected(page, `Expected 2. Bilateral exchange rates is featured as a card`);
    });

    await it.step(`Expected 3. Effective exchange rates is featured as a card`, async () => {
      await verifyExpected(page, `Expected 3. Effective exchange rates is featured as a card`);
    });

    await it.step(`Expected 4. Unchecking chip shows all leaf topics again and unmarked the chip as selected.`, async () => {
      await verifyExpected(page, `Expected 4. Unchecking chip shows all leaf topics again and unmarked the chip as selected.`);
    });
