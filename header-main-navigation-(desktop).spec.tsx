...
test.describe(`Header Main Navigation (Desktop)`, () => {
  test(`Verifies that every item in the main navigation works correctly on Desktop.`, async ({ page }) => {
    await it.step(`1. Go to: \`/\``, async () => {
      await runManualStep(page, `1. Go to: \`/\``);
    });

    await it.step(`2. Click on _Releases_ in main navigation`, async () => {
      await runManualStep(page, `2. Click on _Releases_ in main navigation`);
    });

    await it.step(`Expected 1. Clicking the BIS Logo navigates to \`/\``, async () => {
      await verifyExpected(page, `Expected 1. Clicking the BIS Logo navigates to \`/\``);
    });

    await it.step(`Expected 2. Hovering over _Topics_ opens the Topics Menu: Tested separately #10`, async () => {
      await verifyExpected(page, `Expected 2. Hovering over _Topics_ opens the Topics Menu: Tested separately #10`);
    });

    await it.step(`Expected 3. Clicking on _Releases_ navigates to \`/release-calendar\``, async () => {
      await verifyExpected(page, `Expected 3. Clicking on _Releases_ navigates to \`/release-calendar\``);
    });

    await it.step(`Expected 4. Clicking on _Help_ navigates to \`/help\``, async () => {
      await verifyExpected(page, `Expected 4. Clicking on _Help_ navigates to \`/help\``);
    });

    await it.step(`Expected 5. Clicking on the Ribbon icon opens the _Bookmarks_ modal`, async () => {
      await verifyExpected(page, `Expected 5. Clicking on the Ribbon icon opens the _Bookmarks_ modal`);
    });

    await it.step(`Expected 6. The full screen/normal screen button acts as a toggle (with a screen width of more than 1730px)`, async () => {
      await verifyExpected(page, `Expected 6. The full screen/normal screen button acts as a toggle (with a screen width of more than 1730px)`);
    });

    await it.step(`Expected 7. Home page did not show a _Search button_ in the main navigation`, async () => {
      await verifyExpected(page, `Expected 7. Home page did not show a _Search button_ in the main navigation`);
    });

    await it.step(`Expected 8. The release calendar page shows a _Search_ button in the main navigation: Tested separately #20`, async () => {
      await verifyExpected(page, `Expected 8. The release calendar page shows a _Search_ button in the main navigation: Tested separately #20`);
    });
