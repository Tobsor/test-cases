...
test.describe(`Publication Table Share`, () => {
  test(`A Publication Table should be shareable, also considering the currently configured state.`, async ({ page }) => {
    await it.step(`1. Navigate to \`topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0\``, async () => {
      await runManualStep(page, `1. Navigate to \`topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0\``);
    });

    await it.step(`2. Set the time filter to "2020-Q1"`, async () => {
      await runManualStep(page, `2. Set the time filter to "2020-Q1"`);
    });

    await it.step(`3. Set the Measure filter to "FX and break adjusted change (BIS calculated)"`, async () => {
      await runManualStep(page, `3. Set the Measure filter to "FX and break adjusted change (BIS calculated)"`);
    });

    await it.step(`4. Set the View filter to Series Key`, async () => {
      await runManualStep(page, `4. Set the View filter to Series Key`);
    });

    await it.step(`6. Click on "Share" on the right hand side`, async () => {
      await runManualStep(page, `6. Click on "Share" on the right hand side`);
    });

    await it.step(`3. Clicking on the copy icon copies the link`, async () => {
      await runManualStep(page, `3. Clicking on the copy icon copies the link`);
    });

    await it.step(`4. Open a new tab and paste the link into search bar`, async () => {
      await runManualStep(page, `4. Open a new tab and paste the link into search bar`);
    });
