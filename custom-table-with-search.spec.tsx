...
test.describe(`Custom Table with Search`, () => {
  test(`Build a custom table based on search terms and filters for a specific topic.`, async ({ page }) => {
    ...
    await it.step(`2. Click into search input and type: "Zurich OR Berlin OR Paris"`, async () => {
      await runManualStep(page, `2. Click into search input and type: "Zurich OR Berlin OR Paris"`);
    });

    await it.step(`3. Click on _Compiling agency_ filter`, async () => {
      await runManualStep(page, `3. Click on _Compiling agency_ filter`);
    });

    await it.step(`4. Click on _Private sector_`, async () => {
      await runManualStep(page, `4. Click on _Private sector_`);
    });

    await it.step(`6. Close filter`, async () => {
      await runManualStep(page, `6. Close filter`);
    });

    await it.step(`7. Click on the _Table_ Switch`, async () => {
      await runManualStep(page, `7. Click on the _Table_ Switch`);
    });

    await it.step(`8. Click on table settings`, async () => {
      await runManualStep(page, `8. Click on table settings`);
    });

    await it.step(`9. Click into filter _Seasonal adjustment_`, async () => {
      await runManualStep(page, `9. Click into filter _Seasonal adjustment_`);
    });

    await it.step(`10. Click on _Non seasonally adjusted_`, async () => {
      await runManualStep(page, `10. Click on _Non seasonally adjusted_`);
    });

    await it.step(`11. Click on the _Timespan_ filter`, async () => {
      await runManualStep(page, `11. Click on the _Timespan_ filter`);
    });

    await it.step(`12. Click on _Last ..._`, async () => {
      await runManualStep(page, `12. Click on _Last ..._`);
    });

    await it.step(`13. Click on _Observations_`, async () => {
      await runManualStep(page, `13. Click on _Observations_`);
    });

    await it.step(`14. Select _4_`, async () => {
      await runManualStep(page, `14. Select _4_`);
    });

    await it.step(`15. Close filter`, async () => {
      await runManualStep(page, `15. Close filter`);
    });

    await it.step(`16. Click on _Period/Date sort_`, async () => {
      await runManualStep(page, `16. Click on _Period/Date sort_`);
    });

    await it.step(`17. Select _Ascending_`, async () => {
      await runManualStep(page, `17. Select _Ascending_`);
    });

    await it.step(`18. Click on _Display_`, async () => {
      await runManualStep(page, `18. Click on _Display_`);
    });

    await it.step(`19. Select _Code & Name_`, async () => {
      await runManualStep(page, `19. Select _Code & Name_`);
    });

    await it.step(`20. Move chip _Real estate type_ before _Covered area_ under _Rows_`, async () => {
      await runManualStep(page, `20. Move chip _Real estate type_ before _Covered area_ under _Rows_`);
    });

    await it.step(`21. Refresh page`, async () => {
      await runManualStep(page, `21. Refresh page`);
    });

    await it.step(`22. Click on _Table settings_`, async () => {
      await runManualStep(page, `22. Click on _Table settings_`);
    });

    await it.step(`23. Click observational value in the table`, async () => {
      await runManualStep(page, `23. Click observational value in the table`);
    });

    await it.step(`24. Click on _Back_ next to the bread crumb`, async () => {
      await runManualStep(page, `24. Click on _Back_ next to the bread crumb`);
    });
