...
const it = test;
const TEST_DATA = `The total number of time series must be greater than 20`;

test.describe(`Global search Page structure`, () => {
  test(`Verifies that the global search features the expected elements to manoeuvre the search page`, async ({ page }) => {
    await it.step(`1. Navigate to "/search"`, async () => {
      await runManualStep(page, `1. Navigate to "/search"`);
    });

    await it.step(`2. Click on Timespan filter, select YTD and close the filter`, async () => {
      await runManualStep(page, `2. Click on Timespan filter, select YTD and close the filter`);
    });

    await it.step(`3. Click on \`Relevance\` button in the toolbar`, async () => {
      await runManualStep(page, `3. Click on \`Relevance\` button in the toolbar`);
    });

    await it.step(`4. Click on \`Topics\` suggestion`, async () => {
      await runManualStep(page, `4. Click on \`Topics\` suggestion`);
    });

    await it.step(`5. Click on the sort icon button in the toolbar`, async () => {
      await runManualStep(page, `5. Click on the sort icon button in the toolbar`);
    });

    await it.step(`6. Check the two first time series`, async () => {
      await runManualStep(page, `6. Check the two first time series`);
    });

    await it.step(`7. Check the checkbox in the toolbar`, async () => {
      await runManualStep(page, `7. Check the checkbox in the toolbar`);
    });

    await it.step(`8. Hover on \`Compare time series\` button in the toolbar`, async () => {
      await runManualStep(page, `8. Hover on \`Compare time series\` button in the toolbar`);
    });

    await it.step(`9. Unhover the floating message`, async () => {
      await runManualStep(page, `9. Unhover the floating message`);
    });

    await it.step(`10. Click on the table icon button in the switch view button in the toolbar`, async () => {
      await runManualStep(page, `10. Click on the table icon button in the switch view button in the toolbar`);
    });

    await it.step(`11. Click on the list icon button in the switch view button in the toolbar`, async () => {
      await runManualStep(page, `11. Click on the list icon button in the switch view button in the toolbar`);
    });

    await it.step(`12. Scroll to the navigation bar at the bottom of the list`, async () => {
      await runManualStep(page, `12. Scroll to the navigation bar at the bottom of the list`);
    });

    await it.step(`13. Click on the last page icon button`, async () => {
      await runManualStep(page, `13. Click on the last page icon button`);
    });

    await it.step(`14. Select "10 / page" in the dropdown menu`, async () => {
      await runManualStep(page, `14. Select "10 / page" in the dropdown menu`);
    });
