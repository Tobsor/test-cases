...
test.describe(`Contact Form Integrations`, () => {
  test(`This test should verify that the contact form is correctly integrated into the public Data Portal.`, async ({ page }) => {
    await it.step(`1. Go to [link](https://dataportal.uat.bisinfo.org/help/getting-started)`, async () => {
      await runManualStep(page, `1. Go to [link](https://dataportal.uat.bisinfo.org/help/getting-started)`);
    });

    await it.step(`3. Click on "Contact"`, async () => {
      await runManualStep(page, `3. Click on "Contact"`);
    });

    await it.step(`3. Go to [link](https://dataportal.uat.bisinfo.org/)`, async () => {
      await runManualStep(page, `3. Go to [link](https://dataportal.uat.bisinfo.org/)`);
    });

    await it.step(`4. Scroll to the end of the page until you can see the Footer.`, async () => {
      await runManualStep(page, `4. Scroll to the end of the page until you can see the Footer.`);
    });

    await it.step(`5. Click on "Contact us"`, async () => {
      await runManualStep(page, `5. Click on "Contact us"`);
    });

    await it.step(`6. Go to [link](https://dataportal.uat.bisinfo.org/topics/LBS)`, async () => {
      await runManualStep(page, `6. Go to [link](https://dataportal.uat.bisinfo.org/topics/LBS)`);
    });

    await it.step(`7. Click on the Contact link within the Metadata section of the topic overview page`, async () => {
      await runManualStep(page, `7. Click on the Contact link within the Metadata section of the topic overview page`);
    });
