...
test.describe(`Search suggestions`, () => {
  test(`This issue verifies that the search suggestions work expectedly (including wild cards) and helps the user to find content related to their search term.`, async ({ page }) => {
    await it.step(`1. Navigate to \`/\``, async () => {
      await runManualStep(page, `1. Navigate to \`/\``);
    });

    await it.step(`2. Click into the search bar`, async () => {
      await runManualStep(page, `2. Click into the search bar`);
    });

    await it.step(`3. Type in the term \`Germany\``, async () => {
      await runManualStep(page, `3. Type in the term \`Germany\``);
    });

    await it.step(`4. Click on the item \`Germany\` with the description \`International banking / Consolidated banking statistics\``, async () => {
      await runManualStep(page, `4. Click on the item \`Germany\` with the description \`International banking / Consolidated banking statistics\``);
    });

    await it.step(`5. Open the search bar again by clicking the \`Search\` button in the header.`, async () => {
      await runManualStep(page, `5. Open the search bar again by clicking the \`Search\` button in the header.`);
    });

    await it.step(`6. Type in \`St.\``, async () => {
      await runManualStep(page, `6. Type in \`St.\``);
    });

    await it.step(`8. Type in \`Q\``, async () => {
      await runManualStep(page, `8. Type in \`Q\``);
    });

    await it.step(`9. Type in \`Q.A\``, async () => {
      await runManualStep(page, `9. Type in \`Q.A\``);
    });

    await it.step(`10. Type in \`Q.A.\``, async () => {
      await runManualStep(page, `10. Type in \`Q.A.\``);
    });

    await it.step(`11. Type in the key \`Q.A.B\` and position your cursor after the \`A\`.`, async () => {
      await runManualStep(page, `11. Type in the key \`Q.A.B\` and position your cursor after the \`A\`.`);
    });

    await it.step(`12. Type in the key \`Q.*.B\` and position your cursor after the \`*\``, async () => {
      await runManualStep(page, `12. Type in the key \`Q.*.B\` and position your cursor after the \`*\``);
    });

    await it.step(`13. Type in the key \`Q.A+\` and position your cursor after the \`+\` operator`, async () => {
      await runManualStep(page, `13. Type in the key \`Q.A+\` and position your cursor after the \`+\` operator`);
    });

    await it.step(`14. Type in the term \`Q.A+N.B,A.*\``, async () => {
      await runManualStep(page, `14. Type in the term \`Q.A+N.B,A.*\``);
    });

    await it.step(`15. Hit enter`, async () => {
      await runManualStep(page, `15. Hit enter`);
    });
