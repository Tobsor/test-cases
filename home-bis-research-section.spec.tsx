...
test.describe(`Home BIS research section`, () => {
  test(`Verifies the completeness of this section on the homepage and correct links.`, async ({ page }) => {
    await it.step(`1. Navigate to /`, async () => {
      await runManualStep(page, `1. Navigate to /`);
    });

    await it.step(`2. Scroll to the BIS research section`, async () => {
      await runManualStep(page, `2. Scroll to the BIS research section`);
    });

    await it.step(`Expected 1. Scrolling through the horizontal list, the following items should be visible:`, async () => {
      await verifyExpected(page, `Expected 1. Scrolling through the horizontal list, the following items should be visible:`);
    });

    await it.step(`Expected 2. Clicking on the "View all" Button in the BIS research card opens a new tab to this address: https://www.bis.org/forum/research.htm`, async () => {
      await verifyExpected(page, `Expected 2. Clicking on the "View all" Button in the BIS research card opens a new tab to this address: https://www.bis.org/forum/research.htm`);
    });
