...
test.describe(`Home Page Structure`, () => {
  test(`Ensures that the page structure of the home page is as expected and features the correct components.`, async ({ page }) => {
    await it.step(`1. Navigate to /`, async () => {
      await runManualStep(page, `1. Navigate to /`);
    });

    await it.step(`Expected 1. Page features a search bar`, async () => {
      await verifyExpected(page, `Expected 1. Page features a search bar`);
    });

    await it.step(`Expected 2. Page features quick link chips for every main/parent topic`, async () => {
      await verifyExpected(page, `Expected 2. Page features quick link chips for every main/parent topic`);
    });

    await it.step(`Expected 3. Highlights section tested separately: #597`, async () => {
      await verifyExpected(page, `Expected 3. Highlights section tested separately: #597`);
    });

    await it.step(`Expected 4. Release calendar section tested separately: #13`, async () => {
      await verifyExpected(page, `Expected 4. Release calendar section tested separately: #13`);
    });

    await it.step(`Expected 5. Page features in the BIS Statistics Section cards for selected/highlighted topics`, async () => {
      await verifyExpected(page, `Expected 5. Page features in the BIS Statistics Section cards for selected/highlighted topics`);
    });

    await it.step(`Expected 6. Page features cards in the BIS Research Section:`, async () => {
      await verifyExpected(page, `Expected 6. Page features cards in the BIS Research Section:`);
    });

    await it.step(`Expected 7. Top to page button appears when scrolling down. Click scrolls to top.`, async () => {
      await verifyExpected(page, `Expected 7. Top to page button appears when scrolling down. Click scrolls to top.`);
    });
