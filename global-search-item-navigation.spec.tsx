...
test.describe(`Global Search Item Navigation`, () => {
  test(`Verifies that found search results are interactable or redirect you correctly`, async ({ page }) => {
    ...
    await it.step(`2. Click on a time series in the time series list`, async () => {
      await runManualStep(page, `2. Click on a time series in the time series list`);
    });

    await it.step(`3. Navigate to \`/search\``, async () => {
      await runManualStep(page, `3. Navigate to \`/search\``);
    });

    await it.step(`4. Select two time series in the time series list by checking the checkboxes`, async () => {
      await runManualStep(page, `4. Select two time series in the time series list by checking the checkboxes`);
    });

    await it.step(`5. Click on \`Compare time series\``, async () => {
      await runManualStep(page, `5. Click on \`Compare time series\``);
    });

    await it.step(`6. Navigate to \`/search\``, async () => {
      await runManualStep(page, `6. Navigate to \`/search\``);
    });

    await it.step(`7. Select the table view in the toolbar`, async () => {
      await runManualStep(page, `7. Select the table view in the toolbar`);
    });

    await it.step(`8. Click on a series key of a time series in the list`, async () => {
      await runManualStep(page, `8. Click on a series key of a time series in the list`);
    });

    await it.step(`9. Navigate to \`/search\``, async () => {
      await runManualStep(page, `9. Navigate to \`/search\``);
    });

    await it.step(`10. Select the table view in the toolbar`, async () => {
      await runManualStep(page, `10. Select the table view in the toolbar`);
    });

    await it.step(`11. Select two time series in the time series table by checking the checkboxes`, async () => {
      await runManualStep(page, `11. Select two time series in the time series table by checking the checkboxes`);
    });

    await it.step(`12. Click on \`Compare time series\``, async () => {
      await runManualStep(page, `12. Click on \`Compare time series\``);
    });
