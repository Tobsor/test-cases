...
test.describe(`Time Series Detail Page Structure`, () => {
  test(`Verifies that the page features the expected items, especially the graph to visualize data.`, async ({ page }) => {
    await it.step(`1. navigate to \`/topics/CBPOL/BIS,WS_CBPOL,1.0/M.BR\` and verify the following`, async () => {
      await runManualStep(page, `1. navigate to \`/topics/CBPOL/BIS,WS_CBPOL,1.0/M.BR\` and verify the following`);
    });

    await it.step(`2. Navigate to \`/topics/LBS/BIS,WS_LBS_D_PUB,1.0/Q.S.C.A.TO1.A.5J.A.5A.A.5J.A\``, async () => {
      await runManualStep(page, `2. Navigate to \`/topics/LBS/BIS,WS_LBS_D_PUB,1.0/Q.S.C.A.TO1.A.5J.A.5A.A.5J.A\``);
    });
