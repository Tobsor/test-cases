...
test.describe(`Custom Table with Selection`, () => {
  test(`Build a custom table based on search terms and specific selection`, async ({ page }) => {
    ...
    await it.step(`2. Click on the _Table_ Switch`, async () => {
      await runManualStep(page, `2. Click on the _Table_ Switch`);
    });

    await it.step(`3. Click on _List_ in the view switch`, async () => {
      await runManualStep(page, `3. Click on _List_ in the view switch`);
    });

    await it.step(`4. Click into search input and type: "Zurich OR Berlin OR Paris OR Stockholm"`, async () => {
      await runManualStep(page, `4. Click into search input and type: "Zurich OR Berlin OR Paris OR Stockholm"`);
    });

    await it.step(`5. Click on the _Table_ Switch`, async () => {
      await runManualStep(page, `5. Click on the _Table_ Switch`);
    });

    await it.step(`6. Click on _List_ in the view switch`, async () => {
      await runManualStep(page, `6. Click on _List_ in the view switch`);
    });

    await it.step(`7. Click on the _Timespan_ filter`, async () => {
      await runManualStep(page, `7. Click on the _Timespan_ filter`);
    });

    await it.step(`8. Click on _Years_`, async () => {
      await runManualStep(page, `8. Click on _Years_`);
    });

    await it.step(`9. Click on one year`, async () => {
      await runManualStep(page, `9. Click on one year`);
    });

    await it.step(`10. Close filter`, async () => {
      await runManualStep(page, `10. Close filter`);
    });

    await it.step(`11. Select only time series containing _Stockholm_ or _Zurich_ in its title`, async () => {
      await runManualStep(page, `11. Select only time series containing _Stockholm_ or _Zurich_ in its title`);
    });

    await it.step(`12. Click on the _Table_ view`, async () => {
      await runManualStep(page, `12. Click on the _Table_ view`);
    });

    await it.step(`13. Refresh page`, async () => {
      await runManualStep(page, `13. Refresh page`);
    });

    await it.step(`14. Click on _List_ view`, async () => {
      await runManualStep(page, `14. Click on _List_ view`);
    });
