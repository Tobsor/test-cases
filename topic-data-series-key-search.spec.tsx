...
test.describe(`Topic Data Series Key Search`, () => {
  test(`Verifies that the topic data search bar filters the data items by series key correctly`, async ({ page }) => {
    await it.step(`1. Navigate to \`/topics/EER/data\``, async () => {
      await runManualStep(page, `1. Navigate to \`/topics/EER/data\``);
    });

    await it.step(`2. Identify one item in the list (e.g. \`Nominal effective exchange rate, Algeria / Broad basket\`)`, async () => {
      await runManualStep(page, `2. Identify one item in the list (e.g. \`Nominal effective exchange rate, Algeria / Broad basket\`)`);
    });

    await it.step(`3. Type the series key of the identified item in the search input (e.g. \`M.N.B.DZ\`) and press ENTER`, async () => {
      await runManualStep(page, `3. Type the series key of the identified item in the search input (e.g. \`M.N.B.DZ\`) and press ENTER`);
    });

    await it.step(`4. Click on X (clear) button in the search input`, async () => {
      await runManualStep(page, `4. Click on X (clear) button in the search input`);
    });

    await it.step(`5. Type the series key of the identified item without the reference area code and replace it by \`*\` (e.g. \`M.N.B.*\`)`, async () => {
      await runManualStep(page, `5. Type the series key of the identified item without the reference area code and replace it by \`*\` (e.g. \`M.N.B.*\`)`);
    });

    await it.step(`6. Remove the star \`*\` (e.g. \`M.N.B.\`) from the search input and hit ENTER`, async () => {
      await runManualStep(page, `6. Remove the star \`*\` (e.g. \`M.N.B.\`) from the search input and hit ENTER`);
    });

    await it.step(`7. Click on X (clear) button in the search input`, async () => {
      await runManualStep(page, `7. Click on X (clear) button in the search input`);
    });

    await it.step(`8. Type the series key of the identified item without the type code and replace it by \`*\` (e.g. \`M.*.B.DZ\`)`, async () => {
      await runManualStep(page, `8. Type the series key of the identified item without the type code and replace it by \`*\` (e.g. \`M.*.B.DZ\`)`);
    });

    await it.step(`9. Remove the star and leave it empty (e.g. \`M..B.DZ\`) in the search input and hit ENTER`, async () => {
      await runManualStep(page, `9. Remove the star and leave it empty (e.g. \`M..B.DZ\`) in the search input and hit ENTER`);
    });

    await it.step(`10. Enter \`M.N+R.B.DZ\` into the search field and hit ENTER`, async () => {
      await runManualStep(page, `10. Enter \`M.N+R.B.DZ\` into the search field and hit ENTER`);
    });

    await it.step(`10. Extend the search term by adding \`+AR\` at the end and hit ENTER: \`M.N+R.B.DZ+AR\``, async () => {
      await runManualStep(page, `10. Extend the search term by adding \`+AR\` at the end and hit ENTER: \`M.N+R.B.DZ+AR\``);
    });
