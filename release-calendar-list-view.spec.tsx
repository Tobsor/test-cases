...
test.describe(`Release calendar List view`, () => {
  test(`Verifies that the release calendar list features upcoming releases`, async ({ page }) => {
    await it.step(`1. Navigate to \`/release-calendar?view=list\``, async () => {
      await runManualStep(page, `1. Navigate to \`/release-calendar?view=list\``);
    });

    await it.step(`2. Click on \`Upcoming releases\``, async () => {
      await runManualStep(page, `2. Click on \`Upcoming releases\``);
    });

    await it.step(`3. Click on \`Past releases\``, async () => {
      await runManualStep(page, `3. Click on \`Past releases\``);
    });

    await it.step(`4. Click on \`Show more\` button`, async () => {
      await runManualStep(page, `4. Click on \`Show more\` button`);
    });

    await it.step(`5. Click on \`Upcoming releases\` tab button`, async () => {
      await runManualStep(page, `5. Click on \`Upcoming releases\` tab button`);
    });

    await it.step(`6. Click on \`Release date\` filter and select \`2024-01 to 2024-02\``, async () => {
      await runManualStep(page, `6. Click on \`Release date\` filter and select \`2024-01 to 2024-02\``);
    });

    await it.step(`7. Click on \`Topics\` filter and select \`Effective exchange rates\``, async () => {
      await runManualStep(page, `7. Click on \`Topics\` filter and select \`Effective exchange rates\``);
    });

    await it.step(`8. Refresh the page`, async () => {
      await runManualStep(page, `8. Refresh the page`);
    });

    await it.step(`9. Click on the first release link in the list`, async () => {
      await runManualStep(page, `9. Click on the first release link in the list`);
    });

    await it.step(`10. Go back to the previous Tab with the release calendar`, async () => {
      await runManualStep(page, `10. Go back to the previous Tab with the release calendar`);
    });

    await it.step(`10. Clear \`Release date\` and \`Topics\` filters`, async () => {
      await runManualStep(page, `10. Clear \`Release date\` and \`Topics\` filters`);
    });
