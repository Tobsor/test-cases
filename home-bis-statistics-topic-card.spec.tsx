...
test.describe(`Home BIS statistics topic card`, () => {
  test(`Verifies that the BIS statistic section cards have the correct linkage to the topic`, async ({ page }) => {
    ...
    await it.step(`2. Scroll to the "BIS statistics" section`, async () => {
      await runManualStep(page, `2. Scroll to the "BIS statistics" section`);
    });

    await it.step(`3. Hover on the card with the title "Global liquidity"`, async () => {
      await runManualStep(page, `3. Hover on the card with the title "Global liquidity"`);
    });

    await it.step(`Expected 1. The user should be redirected to /topics/GLI if clicking on the card`, async () => {
      await verifyExpected(page, `Expected 1. The user should be redirected to /topics/GLI if clicking on the card`);
    });

    await it.step(`Expected 2. A short text appears over the card image describing GLI. Image is darkened/blacked out.`, async () => {
      await verifyExpected(page, `Expected 2. A short text appears over the card image describing GLI. Image is darkened/blacked out.`);
    });
