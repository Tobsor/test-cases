...
test.describe(`Time Series Detail Share`, () => {
  test(`Verifies that the time series detail page with its configured state is shareable.`, async ({ page }) => {
    ...
    await it.step(`2. Toggle the "5Y" filter on the left hand side to enable it`, async () => {
      await runManualStep(page, `2. Toggle the "5Y" filter on the left hand side to enable it`);
    });

    await it.step(`3. Select the Observations view on the right side of the graph`, async () => {
      await runManualStep(page, `3. Select the Observations view on the right side of the graph`);
    });

    await it.step(`4. Add the dimensions "Monthly" by clicking on the Frequency Dropdown and select the monthly item`, async () => {
      await runManualStep(page, `4. Add the dimensions "Monthly" by clicking on the Frequency Dropdown and select the monthly item`);
    });

    await it.step(`5. Press the "Share"-button on the right hand side- A modal opens up showing the URL which is shareable`, async () => {
      await runManualStep(page, `5. Press the "Share"-button on the right hand side- A modal opens up showing the URL which is shareable`);
    });

    await it.step(`6. Copy the URL with the copy icon on the right side of the input box.`, async () => {
      await runManualStep(page, `6. Copy the URL with the copy icon on the right side of the input box.`);
    });

    await it.step(`7. Open a new tab and paste the copied url into the search field. Press enter and verify the following:`, async () => {
      await runManualStep(page, `7. Open a new tab and paste the copied url into the search field. Press enter and verify the following:`);
    });
