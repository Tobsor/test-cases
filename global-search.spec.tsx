...
test.describe(`Global search`, () => {
  test(`Verifies that a global search on time series can be triggered by the search bar`, async ({ page }) => {
    await it.step(`1. Go to \`/help\``, async () => {
      await runManualStep(page, `1. Go to \`/help\``);
    });
    ...
    await it.step(`3. Type in: _US Dollar Swiss franc_ and hit ENTER`, async () => {
      await runManualStep(page, `3. Type in: _US Dollar Swiss franc_ and hit ENTER`);
    });

    await it.step(`4. Click on Topic filter`, async () => {
      await runManualStep(page, `4. Click on Topic filter`);
    });

    await it.step(`5. Expand Derivatives`, async () => {
      await runManualStep(page, `5. Expand Derivatives`);
    });

    await it.step(`6. Select Triennial survey`, async () => {
      await runManualStep(page, `6. Select Triennial survey`);
    });

    await it.step(`7. Close the filter`, async () => {
      await runManualStep(page, `7. Close the filter`);
    });

    await it.step(`8. Click on button above the time series list: _Show all data for Triennial Survey_`, async () => {
      await runManualStep(page, `8. Click on button above the time series list: _Show all data for Triennial Survey_`);
    });

    await it.step(`9. Navigate back to global search by using browser back`, async () => {
      await runManualStep(page, `9. Navigate back to global search by using browser back`);
    });

    await it.step(`10. Click into search bar and type: "exchange rate" and hit ENTER`, async () => {
      await runManualStep(page, `10. Click into search bar and type: "exchange rate" and hit ENTER`);
    });

    await it.step(`11. Click into Sort filter labelled as _Relevance_`, async () => {
      await runManualStep(page, `11. Click into Sort filter labelled as _Relevance_`);
    });

    await it.step(`12. Select _Start date_`, async () => {
      await runManualStep(page, `12. Select _Start date_`);
    });

    await it.step(`13. Change order to _Ascending_`, async () => {
      await runManualStep(page, `13. Change order to _Ascending_`);
    });

    await it.step(`14. Scroll down to the bottom of the list and jump to the next page`, async () => {
      await runManualStep(page, `14. Scroll down to the bottom of the list and jump to the next page`);
    });

    await it.step(`15. Hit F5 in the browser`, async () => {
      await runManualStep(page, `15. Hit F5 in the browser`);
    });

    await it.step(`16. Click on _X_ (clear) button in the search input`, async () => {
      await runManualStep(page, `16. Click on _X_ (clear) button in the search input`);
    });

    await it.step(`17. Click on _Show all filters_`, async () => {
      await runManualStep(page, `17. Click on _Show all filters_`);
    });

    await it.step(`18. Click into topic filter and select Consumer prices`, async () => {
      await runManualStep(page, `18. Click into topic filter and select Consumer prices`);
    });

    await it.step(`19. Note the number of the chip as N`, async () => {
      await runManualStep(page, `19. Note the number of the chip as N`);
    });

    await it.step(`20. Click on the button labelled _Show N results_`, async () => {
      await runManualStep(page, `20. Click on the button labelled _Show N results_`);
    });

    await it.step(`21. Click into Search input and type in "CHF/USD" and hit ENTER`, async () => {
      await runManualStep(page, `21. Click into Search input and type in "CHF/USD" and hit ENTER`);
    });
