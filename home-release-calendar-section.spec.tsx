...
test.describe(`Home Release Calendar section`, () => {
  test(`Verifies that the release calendar component is rendered correctly on the home page.`, async ({ page }) => {
    ...
    await it.step(`2. Scroll to the release calendar section`, async () => {
      await runManualStep(page, `2. Scroll to the release calendar section`);
    });

    await it.step(`Expected 1. Latest releases is active / Upcoming releases is inactive`, async () => {
      await verifyExpected(page, `Expected 1. Latest releases is active / Upcoming releases is inactive`);
    });

    await it.step(`Expected 2. Clicking on a heading redirects the user to \`/topic/<topic-id>\``, async () => {
      await verifyExpected(page, `Expected 2. Clicking on a heading redirects the user to \`/topic/<topic-id>\``);
    });

    await it.step(`Expected 3. Clicking on the button below the table "View all" redirects the user to \`/release-calendar\``, async () => {
      await verifyExpected(page, `Expected 3. Clicking on the button below the table "View all" redirects the user to \`/release-calendar\``);
    });

    await it.step(`Expected 4. Upcoming releases shows entries that happen in the future`, async () => {
      await verifyExpected(page, `Expected 4. Upcoming releases shows entries that happen in the future`);
    });
