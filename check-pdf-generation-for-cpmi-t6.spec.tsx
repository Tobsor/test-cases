...
test.describe(`Check Pdf generation for CPMI T6`, () => {
  test(`Explain what this test case is about`, async ({ page }) => {
    await it.step(`1. Go to home page`, async () => {
      await runManualStep(page, `1. Go to home page`);
    });

    await it.step(`2. Hover over "Topics" drop down at top of page`, async () => {
      await runManualStep(page, `2. Hover over "Topics" drop down at top of page`);
    });

    await it.step(`3. Hover/select "Payment Statistics" - currently at end of drop down`, async () => {
      await runManualStep(page, `3. Hover/select "Payment Statistics" - currently at end of drop down`);
    });

    await it.step(`4. Click on the panel "Retail payments, currency and related indicators"`, async () => {
      await runManualStep(page, `4. Click on the panel "Retail payments, currency and related indicators"`);
    });

    await it.step(`5. Click on the tab "Tables & Dashboards"`, async () => {
      await runManualStep(page, `5. Click on the tab "Tables & Dashboards"`);
    });

    await it.step(`6. Click on the panel/rectangle "T6  Value of cashless payments and withdrawal/deposit transactions"`, async () => {
      await runManualStep(page, `6. Click on the panel/rectangle "T6  Value of cashless payments and withdrawal/deposit transactions"`);
    });

    await it.step(`7. Click on the filter for "Reporting country" at the top of the table and select "Australia"`, async () => {
      await runManualStep(page, `7. Click on the filter for "Reporting country" at the top of the table and select "Australia"`);
    });

    await it.step(`8. On the right hand side of the page, just under the banner graphic there will be three clickables, Bookmark, Share and Export. Click on Export.`, async () => {
      await runManualStep(page, `8. On the right hand side of the page, just under the banner graphic there will be three clickables, Bookmark, Share and Export. Click on Export.`);
    });

    await it.step(`9. Select the "PDF" radio button and press the "Export" button at the bottom of the pop up.`, async () => {
      await runManualStep(page, `9. Select the "PDF" radio button and press the "Export" button at the bottom of the pop up.`);
    });
