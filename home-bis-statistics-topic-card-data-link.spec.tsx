...
test.describe(`Home BIS statistics topic card data link`, () => {
  test(`Verifies that the BIS statistics section cards have the correct linkage to the topic data page.`, async ({ page }) => {
    ...
    await it.step(`3. Hover over the topic card with the title "Global liquidity"`, async () => {
      await runManualStep(page, `3. Hover over the topic card with the title "Global liquidity"`);
    });

    await it.step(`Expected 1. Data icon with label is shown on the bottom right corner`, async () => {
      await verifyExpected(page, `Expected 1. Data icon with label is shown on the bottom right corner`);
    });

    await it.step(`Expected 2. The user should be redirected to /topics/GLI/data if clicking on icon`, async () => {
      await verifyExpected(page, `Expected 2. The user should be redirected to /topics/GLI/data if clicking on icon`);
    });
