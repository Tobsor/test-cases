...
const it = test;
const TEST_DATA = `Data provided by data loading process before a release. `;

test.describe(`Bulk downloads Download`, () => {
  test(`Verifies that click on a download does download a file containing data.`, async ({ page }) => {
    await it.step(`1. Go to: /bulkdownload`, async () => {
      await runManualStep(page, `1. Go to: /bulkdownload`);
    });

    await it.step(`2. Click on CSV files of different topics`, async () => {
      await runManualStep(page, `2. Click on CSV files of different topics`);
    });

    await it.step(`Expected 1. Download of selected files are triggered and downloaded to disk in zip files.`, async () => {
      await verifyExpected(page, `Expected 1. Download of selected files are triggered and downloaded to disk in zip files.`);
    });

    await it.step(`Expected 2. User does not leave the page.`, async () => {
      await verifyExpected(page, `Expected 2. User does not leave the page.`);
    });
