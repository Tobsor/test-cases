...
test.describe(`Load empty CPMI publication table after using link`, () => {
  test(`An empty CPMI publication table should be produced, if the configured dimensions do not exist for a different table after switching to it.`, async ({ page }) => {
    await it.step(`1. Go to \`/topics/CPMI_FMI/tables-and-dashboards/BIS,CPMI_T11,1.0\``, async () => {
      await runManualStep(page, `1. Go to \`/topics/CPMI_FMI/tables-and-dashboards/BIS,CPMI_T11,1.0\``);
    });

    await it.step(`2. Click into _Reporting country_ and select _Belgium_`, async () => {
      await runManualStep(page, `2. Click into _Reporting country_ and select _Belgium_`);
    });

    await it.step(`3. Click onto the first observation of the first column`, async () => {
      await runManualStep(page, `3. Click onto the first observation of the first column`);
    });

    await it.step(`4. Verify the year of the latest shown observation in the graph`, async () => {
      await runManualStep(page, `4. Verify the year of the latest shown observation in the graph`);
    });

    await it.step(`5. Use the "Back to Tables" button to get back to the previous table view`, async () => {
      await runManualStep(page, `5. Use the "Back to Tables" button to get back to the previous table view`);
    });

    await it.step(`6. Click into _Change table_ and select _T12_- Table is loaded, but is empty`, async () => {
      await runManualStep(page, `6. Click into _Change table_ and select _T12_- Table is loaded, but is empty`);
    });
