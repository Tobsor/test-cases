...
test.describe(`Exploration Country search`, () => {
  test(`Ensures exploration of countries used/referenced in time series.`, async ({ page }) => {
    ...
    await it.step(`2. Click on the button "Explore" embedded on the right side of the search bar`, async () => {
      await runManualStep(page, `2. Click on the button "Explore" embedded on the right side of the search bar`);
    });

    await it.step(`3. Click on the Tab navigation button "Countries" in the upper left corner of the explore fly out menu`, async () => {
      await runManualStep(page, `3. Click on the Tab navigation button "Countries" in the upper left corner of the explore fly out menu`);
    });
    ...
    await it.step(`4. Click on the letter "G" on the alphabet which is horizontally lined on the upper part of the fly out menu`, async () => {
      await runManualStep(page, `4. Click on the letter "G" on the alphabet which is horizontally lined on the upper part of the fly out menu`);
    });

    await it.step(`5. Click into the input field and type "Germany"`, async () => {
      await runManualStep(page, `5. Click into the input field and type "Germany"`);
    });

    await it.step(`6. Click on the link "Germany"`, async () => {
      await runManualStep(page, `6. Click on the link "Germany"`);
    });

    await it.step(`Expected 1. The user should be redirected to \`/search\``, async () => {
      await verifyExpected(page, `Expected 1. The user should be redirected to \`/search\``);
    });

    await it.step(`Expected 2. The items should relate to the term "Germany"`, async () => {
      await verifyExpected(page, `Expected 2. The items should relate to the term "Germany"`);
    });

    await it.step(`Expected 3. The search bar on top of the page should have the value "Germany" set`, async () => {
      await verifyExpected(page, `Expected 3. The search bar on top of the page should have the value "Germany" set`);
    });
