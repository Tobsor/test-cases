...
test.describe(`Publication Table Page structure`, () => {
  test(`Verifies that the Publication Table features the expected elements.`, async ({ page }) => {
    await it.step(`1. Navigate to \`topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0\` and verify the following:`, async () => {
      await runManualStep(page, `1. Navigate to \`topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0\` and verify the following:`);
    });

    await it.step(`2. Open the publication tables section by clicking it`, async () => {
      await runManualStep(page, `2. Open the publication tables section by clicking it`);
    });

    await it.step(`3. Click on A2 publication table`, async () => {
      await runManualStep(page, `3. Click on A2 publication table`);
    });

    await it.step(`4. Click on Tables & dashboards in the breadcrumb`, async () => {
      await runManualStep(page, `4. Click on Tables & dashboards in the breadcrumb`);
    });

    await it.step(`5. Click on A1 publication table`, async () => {
      await runManualStep(page, `5. Click on A1 publication table`);
    });

    await it.step(`6. Navigate to \`topics/DSS/tables-and-dashboards/BIS,SEC_C1,1.0\``, async () => {
      await runManualStep(page, `6. Navigate to \`topics/DSS/tables-and-dashboards/BIS,SEC_C1,1.0\``);
    });

    await it.step(`7. Open the publication tables section by clicking it`, async () => {
      await runManualStep(page, `7. Open the publication tables section by clicking it`);
    });
