...
test.describe(`Topic Data Page Structure (Desktop)`, () => {
  test(`Verifies that the topic data page features all expected elements to navigate through the data of a given topic.`, async ({ page }) => {
    await it.step(`1. Go to: \`/topics/CPI/data\``, async () => {
      await runManualStep(page, `1. Go to: \`/topics/CPI/data\``);
    });

    await it.step(`Expected 1. Title features Consumer prices`, async () => {
      await verifyExpected(page, `Expected 1. Title features Consumer prices`);
    });

    await it.step(`Expected 2. The topic navigation has Data marked as active`, async () => {
      await verifyExpected(page, `Expected 2. The topic navigation has Data marked as active`);
    });

    await it.step(`Expected 3. Search bar and the buttons Bookmark, Share, Export are featured`, async () => {
      await verifyExpected(page, `Expected 3. Search bar and the buttons Bookmark, Share, Export are featured`);
    });

    await it.step(`Expected 4. Sample table sections is expanded`, async () => {
      await verifyExpected(page, `Expected 4. Sample table sections is expanded`);
    });

    await it.step(`Expected 5. Filters section is collapsed`, async () => {
      await verifyExpected(page, `Expected 5. Filters section is collapsed`);
    });

    await it.step(`Expected 6. Time series list is visible`, async () => {
      await verifyExpected(page, `Expected 6. Time series list is visible`);
    });

    await it.step(`Expected 7. Time series card is visible`, async () => {
      await verifyExpected(page, `Expected 7. Time series card is visible`);
    });

    await it.step(`Expected 8. At the bottom pagination elements are visible`, async () => {
      await verifyExpected(page, `Expected 8. At the bottom pagination elements are visible`);
    });
