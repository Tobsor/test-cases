...
test.describe(`Time Series Detail Add time series`, () => {
  test(`This test verifies that time series can be added to a time series detail page via the + Add time series button and that the search and filter functionalities work correctly.`, async ({ page }) => {
    await it.step(`1. Navigate to \`/topics/LBS/BIS,WS_LBS_D_PUB,1.0/Q.S.C.A.TO1.A.5J.A.5A.A.5J.A\``, async () => {
      await runManualStep(page, `1. Navigate to \`/topics/LBS/BIS,WS_LBS_D_PUB,1.0/Q.S.C.A.TO1.A.5J.A.5A.A.5J.A\``);
    });

    await it.step(`2. Click on the \`+ Add time series\` button`, async () => {
      await runManualStep(page, `2. Click on the \`+ Add time series\` button`);
    });

    await it.step(`3. Search for \`Q.S.C.A.TO1.A.5J.A.5A.A.5J.A\``, async () => {
      await runManualStep(page, `3. Search for \`Q.S.C.A.TO1.A.5J.A.5A.A.5J.A\``);
    });

    await it.step(`4. Clear the search by clicking the \`X\` button`, async () => {
      await runManualStep(page, `4. Clear the search by clicking the \`X\` button`);
    });

    await it.step(`5. Click on the \`Filters\` button`, async () => {
      await runManualStep(page, `5. Click on the \`Filters\` button`);
    });

    await it.step(`6. Click on the \`Topic\` filter, expand "International banking," and select "Locational banking statistics"`, async () => {
      await runManualStep(page, `6. Click on the \`Topic\` filter, expand "International banking," and select "Locational banking statistics"`);
    });

    await it.step(`7. Click on the \`Back\` button`, async () => {
      await runManualStep(page, `7. Click on the \`Back\` button`);
    });

    await it.step(`8. Type "Germany" in the search bar and hit ENTER`, async () => {
      await runManualStep(page, `8. Type "Germany" in the search bar and hit ENTER`);
    });

    await it.step(`9. Click on the title of the first checkable time series`, async () => {
      await runManualStep(page, `9. Click on the title of the first checkable time series`);
    });

    await it.step(`10. Click on the \`Update selection\` button`, async () => {
      await runManualStep(page, `10. Click on the \`Update selection\` button`);
    });

    await it.step(`11. Click on the close button on the second time series card`, async () => {
      await runManualStep(page, `11. Click on the close button on the second time series card`);
    });
