...
test.describe(`Topics Overview Page`, () => {
  test(`Check for complete list of topics.`, async ({ page }) => {
    await it.step(`1. Go to \`/topics\``, async () => {
      await runManualStep(page, `1. Go to \`/topics\``);
    });

    await it.step(`Expected 1. Every top level / parent topic is featured as a chip: _Number of top level items 10 (as of 31.07.2024)_`, async () => {
      await verifyExpected(page, `Expected 1. Every top level / parent topic is featured as a chip: _Number of top level items 10 (as of 31.07.2024)_`);
    });

    await it.step(`Expected 2. Every leaf topic is featured and navigates to its topic page \`/topic/<topic_id>\`: _Number of leaf items 20 (as of 31.07.2024)_`, async () => {
      await verifyExpected(page, `Expected 2. Every leaf topic is featured and navigates to its topic page \`/topic/<topic_id>\`: _Number of leaf items 20 (as of 31.07.2024)_`);
    });

    await it.step(`Expected 3. Cards of leaf topics are ordered by top level topic.`, async () => {
      await verifyExpected(page, `Expected 3. Cards of leaf topics are ordered by top level topic.`);
    });
