...
test.describe(`Time series Detail Export`, () => {
  test(`Verifies that the data of a timeseries is exportable`, async ({ page }) => {
    await it.step(`1. navigate to \`/topics/CBPOL/BIS,WS_CBPOL,1.0/D.BR\``, async () => {
      await runManualStep(page, `1. navigate to \`/topics/CBPOL/BIS,WS_CBPOL,1.0/D.BR\``);
    });

    await it.step(`2. Click on Reference area on the right and add _North Macedonia_`, async () => {
      await runManualStep(page, `2. Click on Reference area on the right and add _North Macedonia_`);
    });

    await it.step(`3. Press on the "Export" button on the right hand side`, async () => {
      await runManualStep(page, `3. Press on the "Export" button on the right hand side`);
    });

    await it.step(`4. Click the export button`, async () => {
      await runManualStep(page, `4. Click the export button`);
    });

    await it.step(`5. Close the modal menu`, async () => {
      await runManualStep(page, `5. Close the modal menu`);
    });

    await it.step(`6. In the list below the graph click on text \`D.MK\` (not the list element itself - this might deselect it)`, async () => {
      await runManualStep(page, `6. In the list below the graph click on text \`D.MK\` (not the list element itself - this might deselect it)`);
    });

    await it.step(`7. Remove \`D.BR\` from the list by clicking (x)`, async () => {
      await runManualStep(page, `7. Remove \`D.BR\` from the list by clicking (x)`);
    });

    await it.step(`8. On the left hand side select the filter option "1Y" to reduce the amount of selected data`, async () => {
      await runManualStep(page, `8. On the left hand side select the filter option "1Y" to reduce the amount of selected data`);
    });

    await it.step(`9. Reopen the modal with the "Export" button and trigger the export with the "Export" in the modal.`, async () => {
      await runManualStep(page, `9. Reopen the modal with the "Export" button and trigger the export with the "Export" in the modal.`);
    });
