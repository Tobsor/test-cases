...
test.describe(`Publication Table Export`, () => {
  test(`The data of a publication table should be exportable`, async ({ page }) => {
    await it.step(`1. Go to \`/topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0\``, async () => {
      await runManualStep(page, `1. Go to \`/topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0\``);
    });

    await it.step(`2. Select the time filter _2020-Q1_`, async () => {
      await runManualStep(page, `2. Select the time filter _2020-Q1_`);
    });

    await it.step(`3. Select the Measure _FX and break adjusted change (BIS calculated)_`, async () => {
      await runManualStep(page, `3. Select the Measure _FX and break adjusted change (BIS calculated)_`);
    });

    await it.step(`4. Click on Export`, async () => {
      await runManualStep(page, `4. Click on Export`);
    });

    await it.step(`5. Click on Export`, async () => {
      await runManualStep(page, `5. Click on Export`);
    });

    await it.step(`6. Open downloaded file`, async () => {
      await runManualStep(page, `6. Open downloaded file`);
    });

    await it.step(`7. Click on Export`, async () => {
      await runManualStep(page, `7. Click on Export`);
    });

    await it.step(`8. Select _CSV_ as file format`, async () => {
      await runManualStep(page, `8. Select _CSV_ as file format`);
    });

    await it.step(`9. Select the variant _Data_ and then _Dimension codes_ for additional time series information`, async () => {
      await runManualStep(page, `9. Select the variant _Data_ and then _Dimension codes_ for additional time series information`);
    });
    ...
    await it.step(`11. Open downloaded file`, async () => {
      await runManualStep(page, `11. Open downloaded file`);
    });

    await it.step(`12. Copy the URL listed under _Source URL_`, async () => {
      await runManualStep(page, `12. Copy the URL listed under _Source URL_`);
    });

    await it.step(`13. Open a new browser tab, paste the URL and hit ENTER`, async () => {
      await runManualStep(page, `13. Open a new browser tab, paste the URL and hit ENTER`);
    });

    await it.step(`14. Click on _Export_`, async () => {
      await runManualStep(page, `14. Click on _Export_`);
    });

    await it.step(`15. Select the variant "Data" and then _Descriptive Labels_ under _Additional time series information_`, async () => {
      await runManualStep(page, `15. Select the variant "Data" and then _Descriptive Labels_ under _Additional time series information_`);
    });
    ...
    await it.step(`18. In the sheet _Summary_, click on _Download as Excel_ under _Download URL_`, async () => {
      await runManualStep(page, `18. In the sheet _Summary_, click on _Download as Excel_ under _Download URL_`);
    });

    await it.step(`19. Repeat Step 17`, async () => {
      await runManualStep(page, `19. Repeat Step 17`);
    });
