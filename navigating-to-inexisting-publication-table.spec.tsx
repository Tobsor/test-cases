...
test.describe(`Navigating to inexisting publication table`, () => {
  test(`An empty publication table should be loaded if it does not exist. There are circumstances that dimensions are set before navigating to a different table, which does not exist under this configuration. A table should be still loaded, but it should be empty.`, async ({ page }) => {
    await it.step(`1. Go to \`/topics/DSS\``, async () => {
      await runManualStep(page, `1. Go to \`/topics/DSS\``);
    });

    await it.step(`2. Select _Tables & Dashboards_`, async () => {
      await runManualStep(page, `2. Select _Tables & Dashboards_`);
    });

    await it.step(`3. Click on the tab _Country_`, async () => {
      await runManualStep(page, `3. Click on the tab _Country_`);
    });

    await it.step(`4. Select C5`, async () => {
      await runManualStep(page, `4. Select C5`);
    });

    await it.step(`5. Click into _Reference area_ and select _Euro area - 20, 2023_`, async () => {
      await runManualStep(page, `5. Click into _Reference area_ and select _Euro area - 20, 2023_`);
    });

    await it.step(`6. Scroll down the table and click on the link _International debt securities amounts outstanding, BIS-compiled, in US dollars_`, async () => {
      await runManualStep(page, `6. Scroll down the table and click on the link _International debt securities amounts outstanding, BIS-compiled, in US dollars_`);
    });
