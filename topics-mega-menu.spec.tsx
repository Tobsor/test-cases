...
test.describe(`Topics mega menu`, () => {
  test(`Allows navigating to a topic via topics mega menu.`, async ({ page }) => {
    await it.step(`1. Go to \`/\``, async () => {
      await runManualStep(page, `1. Go to \`/\``);
    });

    await it.step(`2. Hover over the navigation item "Topics"`, async () => {
      await runManualStep(page, `2. Hover over the navigation item "Topics"`);
    });

    await it.step(`3. Hover over the term "Property prices"`, async () => {
      await runManualStep(page, `3. Hover over the term "Property prices"`);
    });

    await it.step(`Expected 1. All top level / parent topics are listed on the left: _10 items are expected (as of 02.05.2024)_`, async () => {
      await verifyExpected(page, `Expected 1. All top level / parent topics are listed on the left: _10 items are expected (as of 02.05.2024)_`);
    });

    await it.step(`Expected 2. Mega menu is separated in master detail view`, async () => {
      await verifyExpected(page, `Expected 2. Mega menu is separated in master detail view`);
    });
