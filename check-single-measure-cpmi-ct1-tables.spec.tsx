...
test.describe(`Check single measure CPMI CT1 tables`, () => {
  test(`There is a CPMI CT1 overview table - comparative table for payments.`, async ({ page }) => {
    await it.step(`1. Go to \`/\``, async () => {
      await runManualStep(page, `1. Go to \`/\``);
    });

    await it.step(`2. Click topics and hover over "Payment Statistics", then click on the card for "Retail payments, currency and related indicators"`, async () => {
      await runManualStep(page, `2. Click topics and hover over "Payment Statistics", then click on the card for "Retail payments, currency and related indicators"`);
    });

    await it.step(`3. Click on the "Tables & dashboards" tab`, async () => {
      await runManualStep(page, `3. Click on the "Tables & dashboards" tab`);
    });

    await it.step(`4. Choose the "Comparative" button under "Tables" on the main part of the page`, async () => {
      await runManualStep(page, `4. Choose the "Comparative" button under "Tables" on the main part of the page`);
    });

    await it.step(`5. Click on CT1 Basic statistical data`, async () => {
      await runManualStep(page, `5. Click on CT1 Basic statistical data`);
    });

    await it.step(`6. You should see a populated table`, async () => {
      await runManualStep(page, `6. You should see a populated table`);
    });

    await it.step(`7. Click on the column header "CPI Inflation"`, async () => {
      await runManualStep(page, `7. Click on the column header "CPI Inflation"`);
    });

    await it.step(`8. Click the back button on the browser to return to the CT1 overview table`, async () => {
      await runManualStep(page, `8. Click the back button on the browser to return to the CT1 overview table`);
    });

    await it.step(`9. Click on the column header for the first column "GDP"`, async () => {
      await runManualStep(page, `9. Click on the column header for the first column "GDP"`);
    });
