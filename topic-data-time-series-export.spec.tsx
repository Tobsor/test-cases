...
test.describe(`Topic Data Time Series Export`, () => {
  test(`Verifies that time series on topic data can be exported.`, async ({ page }) => {
    await it.step(`1. Go to: \`/topics/CPP/data\``, async () => {
      await runManualStep(page, `1. Go to: \`/topics/CPP/data\``);
    });

    await it.step(`2. Search for _Switzerland OR Germany_`, async () => {
      await runManualStep(page, `2. Search for _Switzerland OR Germany_`);
    });

    await it.step(`3. Click on Filter _Covered area_`, async () => {
      await runManualStep(page, `3. Click on Filter _Covered area_`);
    });

    await it.step(`4. Select _Whole country_ as the filter value`, async () => {
      await runManualStep(page, `4. Select _Whole country_ as the filter value`);
    });

    await it.step(`5. Click on _Time Span_ Filter`, async () => {
      await runManualStep(page, `5. Click on _Time Span_ Filter`);
    });

    await it.step(`6. Click on _Last ..._`, async () => {
      await runManualStep(page, `6. Click on _Last ..._`);
    });

    await it.step(`7. Select _Observations_`, async () => {
      await runManualStep(page, `7. Select _Observations_`);
    });

    await it.step(`8. Click on _8_ and close Filter`, async () => {
      await runManualStep(page, `8. Click on _8_ and close Filter`);
    });

    await it.step(`9. Keep note of the number of time series shown in the list`, async () => {
      await runManualStep(page, `9. Keep note of the number of time series shown in the list`);
    });

    await it.step(`10. Click on Export`, async () => {
      await runManualStep(page, `10. Click on Export`);
    });

    await it.step(`11. Hover over Γä╣ next to _Long (stacked)_`, async () => {
      await runManualStep(page, `11. Hover over Γä╣ next to _Long (stacked)_`);
    });

    await it.step(`12. Hover over Γä╣ next to _Wide (unstacked)_`, async () => {
      await runManualStep(page, `12. Hover over Γä╣ next to _Wide (unstacked)_`);
    });

    await it.step(`13. Hover over Γä╣ next to _Dimension codes_`, async () => {
      await runManualStep(page, `13. Hover over Γä╣ next to _Dimension codes_`);
    });

    await it.step(`14. Click on _Wide (unstacked)_`, async () => {
      await runManualStep(page, `14. Click on _Wide (unstacked)_`);
    });

    await it.step(`15. Click on Transposed`, async () => {
      await runManualStep(page, `15. Click on Transposed`);
    });

    await it.step(`16. Click on Export`, async () => {
      await runManualStep(page, `16. Click on Export`);
    });

    await it.step(`17. Open downloaded file`, async () => {
      await runManualStep(page, `17. Open downloaded file`);
    });

    await it.step(`18. Click on Export`, async () => {
      await runManualStep(page, `18. Click on Export`);
    });

    await it.step(`19. Select _CSV_ as file format`, async () => {
      await runManualStep(page, `19. Select _CSV_ as file format`);
    });

    await it.step(`20. Select _Dimension codes_ for additional time series information`, async () => {
      await runManualStep(page, `20. Select _Dimension codes_ for additional time series information`);
    });

    await it.step(`21. Click on Export`, async () => {
      await runManualStep(page, `21. Click on Export`);
    });

    await it.step(`22. Open downloaded file`, async () => {
      await runManualStep(page, `22. Open downloaded file`);
    });

    await it.step(`23. Copy the URL listed under _Source URL_`, async () => {
      await runManualStep(page, `23. Copy the URL listed under _Source URL_`);
    });

    await it.step(`24. Open a new browser tab, paste the URL and hit ENTER`, async () => {
      await runManualStep(page, `24. Open a new browser tab, paste the URL and hit ENTER`);
    });

    await it.step(`25. Check one time series in the list`, async () => {
      await runManualStep(page, `25. Check one time series in the list`);
    });

    await it.step(`26. Click on _Export_`, async () => {
      await runManualStep(page, `26. Click on _Export_`);
    });

    await it.step(`27. Click on _Wide_ as the Format`, async () => {
      await runManualStep(page, `27. Click on _Wide_ as the Format`);
    });

    await it.step(`28. Click on _Descriptive Labels_ under _Additional time series information_`, async () => {
      await runManualStep(page, `28. Click on _Descriptive Labels_ under _Additional time series information_`);
    });

    await it.step(`29. Click on Export`, async () => {
      await runManualStep(page, `29. Click on Export`);
    });

    await it.step(`30. Open downloaded file`, async () => {
      await runManualStep(page, `30. Open downloaded file`);
    });

    await it.step(`31. Click on _Download as Excel_ under _Download URL_`, async () => {
      await runManualStep(page, `31. Click on _Download as Excel_ under _Download URL_`);
    });

    await it.step(`32. Repeat Step 30`, async () => {
      await runManualStep(page, `32. Repeat Step 30`);
    });
