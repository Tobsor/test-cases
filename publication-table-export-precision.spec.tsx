...
const it = test;
const TEST_DATA = `Publication tables have to be deployed see above.`;

test.describe(`Publication Table Export Precision`, () => {
  test(`Making sure that the publication table export contains full (more detailed) precision compared to the tables.`, async ({ page }) => {
    await it.step(`1. Go to the following table LBS A1: /topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0`, async () => {
      await runManualStep(page, `1. Go to the following table LBS A1: /topics/LBS/tables-and-dashboards/BIS,LBS_A1,1.0`);
    });

    await it.step(`2. Click on Export`, async () => {
      await runManualStep(page, `2. Click on Export`);
    });

    await it.step(`3. Select Table and Excel and click on Export`, async () => {
      await runManualStep(page, `3. Select Table and Excel and click on Export`);
    });

    await it.step(`3. Close excel and navigate to: /topics/DSS/tables-and-dashboards/BIS,SEC_C2,1.0`, async () => {
      await runManualStep(page, `3. Close excel and navigate to: /topics/DSS/tables-and-dashboards/BIS,SEC_C2,1.0`);
    });
    ...
    await it.step(`5. Select Table and CSV and click on Export`, async () => {
      await runManualStep(page, `5. Select Table and CSV and click on Export`);
    });

    await it.step(`6. Close the file`, async () => {
      await runManualStep(page, `6. Close the file`);
    });

    await it.step(`7. Click on export again`, async () => {
      await runManualStep(page, `7. Click on export again`);
    });

    await it.step(`8. Select PDF before clicking Export`, async () => {
      await runManualStep(page, `8. Select PDF before clicking Export`);
    });

    await it.step(`9. Go to the following table: /topics/DSS/tables-and-dashboards/BIS,SEC_C1,1.0`, async () => {
      await runManualStep(page, `9. Go to the following table: /topics/DSS/tables-and-dashboards/BIS,SEC_C1,1.0`);
    });
    ...
    await it.step(`11. Select Table and Excel before clicking export`, async () => {
      await runManualStep(page, `11. Select Table and Excel before clicking export`);
    });
