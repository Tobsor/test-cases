...
test.describe(`Custom Table with Filters`, () => {
  test(`Configures a custom table for a specific topic using just filters.`, async ({ page }) => {
    ...
    await it.step(`2. Click on _Reference area_ filter`, async () => {
      await runManualStep(page, `2. Click on _Reference area_ filter`);
    });

    await it.step(`3. Filter for _Sw_`, async () => {
      await runManualStep(page, `3. Filter for _Sw_`);
    });

    await it.step(`4. Select _Sweden_ and _Switzerland_`, async () => {
      await runManualStep(page, `4. Select _Sweden_ and _Switzerland_`);
    });

    await it.step(`5. Click on _Unit of measure_ filter`, async () => {
      await runManualStep(page, `5. Click on _Unit of measure_ filter`);
    });

    await it.step(`6. Select _Index, 2010 = 100_`, async () => {
      await runManualStep(page, `6. Select _Index, 2010 = 100_`);
    });

    await it.step(`7. Close filter`, async () => {
      await runManualStep(page, `7. Close filter`);
    });

    await it.step(`8. Click on _Table_ Switch`, async () => {
      await runManualStep(page, `8. Click on _Table_ Switch`);
    });

    await it.step(`9. Click on _Table Settings_`, async () => {
      await runManualStep(page, `9. Click on _Table Settings_`);
    });

    await it.step(`10. Move _Period_ to _Rows_`, async () => {
      await runManualStep(page, `10. Move _Period_ to _Rows_`);
    });

    await it.step(`11. Move _Frequency_ and _Reference Area_ to _Columns_`, async () => {
      await runManualStep(page, `11. Move _Frequency_ and _Reference Area_ to _Columns_`);
    });

    await it.step(`12. Make sure _Reference area_ comes before _Frequency_`, async () => {
      await runManualStep(page, `12. Make sure _Reference area_ comes before _Frequency_`);
    });
