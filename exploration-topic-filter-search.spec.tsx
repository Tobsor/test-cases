...
test.describe(`Exploration Topic Filter Search`, () => {
  test(`Ensures exploration can drill down to filters of a topic and triggers a search.`, async ({ page }) => {
    ...
    await it.step(`2. Click the button "Explore" embedded to the right side within the search bar`, async () => {
      await runManualStep(page, `2. Click the button "Explore" embedded to the right side within the search bar`);
    });

    await it.step(`3. Select the item "Credit" in the left Pane`, async () => {
      await runManualStep(page, `3. Select the item "Credit" in the left Pane`);
    });

    await it.step(`4. Select the item "Debt service ratios" in the next Pane`, async () => {
      await runManualStep(page, `4. Select the item "Debt service ratios" in the next Pane`);
    });

    await it.step(`5. Select the item "Borrowers" in the next Pane`, async () => {
      await runManualStep(page, `5. Select the item "Borrowers" in the next Pane`);
    });

    await it.step(`6. Check the checkbox next to "Non-financial corporations" by clicking it`, async () => {
      await runManualStep(page, `6. Check the checkbox next to "Non-financial corporations" by clicking it`);
    });

    await it.step(`7. Click on Reset (next to "Show x results")`, async () => {
      await runManualStep(page, `7. Click on Reset (next to "Show x results")`);
    });

    await it.step(`8. Repeat Step 5`, async () => {
      await runManualStep(page, `8. Repeat Step 5`);
    });

    await it.step(`9. Click on Select all`, async () => {
      await runManualStep(page, `9. Click on Select all`);
    });

    await it.step(`10. Click on Reset (above the filter items)`, async () => {
      await runManualStep(page, `10. Click on Reset (above the filter items)`);
    });

    await it.step(`11. Click on "Non-financial corporations"`, async () => {
      await runManualStep(page, `11. Click on "Non-financial corporations"`);
    });

    await it.step(`12. Press on the button labelled "Show X results"`, async () => {
      await runManualStep(page, `12. Press on the button labelled "Show X results"`);
    });
