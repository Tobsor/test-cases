...
test.describe(`Contact Form Basic use case`, () => {
  test(`Explain what this test case is about`, async ({ page }) => {
    await it.step(`1. Go to [link](https://dataportal.uat.bisinfo.org/help/contact)`, async () => {
      await runManualStep(page, `1. Go to [link](https://dataportal.uat.bisinfo.org/help/contact)`);
    });

    await it.step(`2. Click on the underlined text "Contact us" below the useful link cards`, async () => {
      await runManualStep(page, `2. Click on the underlined text "Contact us" below the useful link cards`);
    });

    await it.step(`3. Click into the first name field`, async () => {
      await runManualStep(page, `3. Click into the first name field`);
    });

    await it.step(`4. Enter the following Data:`, async () => {
      await runManualStep(page, `4. Enter the following Data:`);
    });

    await it.step(`5. Select the "Central bank statistics" in the topic selection`, async () => {
      await runManualStep(page, `5. Select the "Central bank statistics" in the topic selection`);
    });

    await it.step(`6. Select the subtopic "Central bank policy rates"`, async () => {
      await runManualStep(page, `6. Select the subtopic "Central bank policy rates"`);
    });

    await it.step(`7. Change the topic to "Credit"`, async () => {
      await runManualStep(page, `7. Change the topic to "Credit"`);
    });

    await it.step(`8. Change the topic to "Consumer prices"`, async () => {
      await runManualStep(page, `8. Change the topic to "Consumer prices"`);
    });

    await it.step(`9. Reselect the topic "Central bank statistics" and the subtopic "Central bank policy rates"`, async () => {
      await runManualStep(page, `9. Reselect the topic "Central bank statistics" and the subtopic "Central bank policy rates"`);
    });

    await it.step(`10. Add the remaining missing data as follows:`, async () => {
      await runManualStep(page, `10. Add the remaining missing data as follows:`);
    });

    await it.step(`11. Press the submit button`, async () => {
      await runManualStep(page, `11. Press the submit button`);
    });

    await it.step(`12. Open your personal Outlook`, async () => {
      await runManualStep(page, `12. Open your personal Outlook`);
    });
